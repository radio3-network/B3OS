...
void print(WRState* w, const WRValue* argv, const int argn, WRValue& retVal, void* usr) {
    ...
    for (int i = 0; i < argn; ++i) {
        response1 -> printf("%s", argv[i].asString(buf, 1024));
    }
}
...
void func_parse_c_script(char* args, Stream* response) {
  ...
  {
    response -> print( "Compiling error: ");
    response -> println( err);
    response -> print( "Did you forget to remove the #define WRENCH_WITHOUT_COMPILER");
  }
